  ...
  try {
    ...
    app.get("/product", async (req, res) => {
      try {
        ...
        // Default query and options for fetching all data
        let query = {};
        let options = {};
        ...
        // If sort parameter is provided, add it to the options
        if (filter?.sort) {
          options.sort = { price: filter.sort === "asc" ? 1 : -1 };
        }
    
        // Fetch data based on the constructed query and options
    ...
    app.post("/carts", async (req, res) => {
      ...
      const inventoryItem = await productCollection.findOne({
